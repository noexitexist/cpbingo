...
// 1. 웹 화면에 빈 칸 생성 및 아이폰 사파리 우회 파일 업로드
function createGrid(gridElement, imageArray) {
    for (let i = 0; i < 12; i++) {
        ...
        cell.addEventListener('click', () => {
            // 💡 [해결 포인트] 터치하자마자 인풋을 만들고 다른 작업 없이 '곧바로' 클릭해야 사진첩이 무조건 열립니다.
            ...
            fileInput.onchange = (e) => {
                ...
                reader.onload = (event) => {
                    ...
                    // 이미지 로드가 완료되면 크러퍼 실행 (모바일 최적화 세팅)
                    cropperImage.onload = () => {
                        if (cropper) return;
...
cropConfirmBtn.addEventListener('click', () => {
    ...
    // 1020x850 해상도에 부드럽고 선명하게 들어갈 300px로 크롭 추출
...
// 2. 이미지 합성 및 다운로드 함수 (1020x850 리사이징 완벽 매핑 좌표)
downloadBtn.addEventListener('click', () => {
    const templateImg = new Image();
    templateImg.src = 'template.png'; 

    templateImg.onload = () => {
        // 캔버스 크기를 1020 x 850으로 강제 고정
        canvas.width = 1020;
        canvas.height = 850;
        ...
        const gridY = 135;       // 타이틀 아래 그리드 시작 Y 좌표
